...
export const storefrontRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get("/:tenantSlug", async (req, reply) => {
    ...
    // Narx uchta joyda bo'ladi: mahsulotning o'zida, variantlarda va
    // hisoblangan `pricing` da. Uchalasi ham kesilishi shart — aks holda
    // ulgurji narx `variants[]` orqali baribir ko'rinib qolardi.
    const publicProducts = b2bHidesPrices
      ? enrichedProducts.map((p) => ({
          ...
          oldPrice: null,
          comboAddons: [],
          variants: p.variants.map((v) => ({ ...v, price: 0, oldPrice: null })),
          pricing: { ...p.pricing, price: 0, oldPrice: null, maxPrice: 0, priceVaries: false },
        }))
      : enrichedProducts;
  ...
  app.get("/:tenantSlug/profile", async (req, reply) => {
    ...
    const auth = await authStorefrontCustomer(app.prisma as never, tenant.id, q.tgUserId, q.initData);
    if (!auth.ok) return reply.code(auth.code).send({ error: auth.error });
  ...
  app.patch("/:tenantSlug/profile", async (req, reply) => {
    ...
    const auth = await authStorefrontCustomer(app.prisma as never, tenant.id, data.tgUserId, data.initData);
    if (!auth.ok) return reply.code(auth.code).send({ error: auth.error });
  ...
  app.patch("/:tenantSlug/addresses/:id", async (req, reply) => {
    ...
    const auth = await authStorefrontCustomer(app.prisma as never, tenant.id, tgUserId, data.initData);
    if (!auth.ok) return reply.code(auth.code).send({ error: auth.error });
