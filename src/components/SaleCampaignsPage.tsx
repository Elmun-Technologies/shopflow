    ...
    <>
      ...
      {loading ? (
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl py-20 px-6 text-center">
          ...
          <h3 className="text-lg font-semibold text-white mb-1">Aksiyalar yo'q</h3>
          ...
          {list.map((c) => (
            <div key={c.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-4 hover:border-slate-700 transition-colors">
              <div className="flex items-start justify-between gap-3 mb-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {c.active ? (
                      <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-300">Faol</span>
                    ) : (
                      <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-slate-800 text-slate-500">O'chirilgan</span>
                    )}
                  </div>
              ...
              <div className="flex items-center justify-end gap-1 border-t border-slate-800 pt-2">
                <button onClick={() => toggleActive(c)} className="p-1.5 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800" title={c.active ? "Yashirish" : "Yoqish"}>
                  {c.active ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                </button>
                <button onClick={() => setEditingId(c.id)} className="p-1.5 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800" title="Tahrirlash">
                  <Edit2 className="w-4 h-4" />
                </button>
                <button onClick={() => remove(c)} className="p-1.5 rounded-lg text-slate-400 hover:text-rose-400 hover:bg-rose-500/10" title="O'chirish">
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[200] p-4" onClick={onClose}>
      ...
      >
        <div className="sticky top-0 bg-slate-900 border-b border-slate-800 px-5 py-3 flex items-center justify-between z-10">
          <h2 className="text-base font-semibold text-white">{isEdit ? "Aksiya tahrirlash" : "Yangi aksiya"}</h2>
        ...
        {loading ? (
          ...
          <div className="p-5 space-y-3">
            ...
            <div className="pt-3 border-t border-slate-800">
              ...
              <div className="border border-slate-800 rounded-lg max-h-60 overflow-y-auto divide-y divide-slate-800">
                {filteredProducts.length === 0 ? (
                  <p className="text-xs text-slate-500 p-4 text-center">Mahsulot topilmadi</p>
                ) : (
                  filteredProducts.map((p) => (
                    <label key={p.id} className="flex items-center gap-3 px-3 py-2 hover:bg-slate-800/50 cursor-pointer">
                      <input
                        type="checkbox"
                      ...
                      <div className="text-xs text-slate-400">{Number(p.price).toLocaleString("uz-UZ")}</div>
                    </label>
                  ))
                )}
              </div>
        ...
        <div className="sticky bottom-0 bg-slate-900 border-t border-slate-800 px-5 py-3 flex gap-2">
          <button onClick={onClose} className="flex-1 py-2.5 text-sm text-slate-300 bg-slate-800 hover:bg-slate-700 rounded-lg font-medium">
            Bekor
          </button>
          <button
            onClick={save}
