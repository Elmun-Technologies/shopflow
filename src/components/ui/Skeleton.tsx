...
export function SkelLine({ w = "100%", h = 12 }: { w?: string | number; h?: number }) {
  return (
    <div
      className="bg-slate-800 rounded-full animate-pulse"
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      {Array.from({ length: 4 }).map((_, i) => (
        <div key={i} className="bg-slate-800/50 border border-slate-800 rounded-2xl p-5">
          ...
          <SkelLine w={120} h={14} />
          <div className="h-7 mt-2 mb-1 bg-slate-800 rounded-full w-3/4 animate-pulse" />
          <SkelLine w="50%" h={10} />
        </div>
      ))}
    </div>
  ...
  return (
    <div className="space-y-1">
      {Array.from({ length: rows }).map((_, ri) => (
        <div
          key={ri}
          className="grid gap-3 py-3 px-4 border-b border-slate-800/40"
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-slate-800/30 border border-slate-800 rounded-2xl p-5">
        <div className="h-5 w-32 bg-slate-800 rounded-lg animate-pulse mb-4" />
...
export function PanelSkeleton({ h = 280 }: { h?: number }) {
  return (
    <div className="bg-slate-800/30 border border-slate-800 rounded-2xl p-5">
      ...
      <div className="bg-slate-800/40 rounded-xl animate-pulse" style={{ height: h }} />
    </div>
  );
}
