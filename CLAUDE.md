...
- [ ] **Storefront "legacy mode" auth** (`telegram-auth.ts`) — tenant'ning Telegram kanali FAOL emas (yoki token bo'sh) bo'lsa `authStorefrontCustomer` initData HMAC tekshiruvini o'tkazib yuboradi (`ok:true`) → `tgUserId` spoofable, profil/orders/addresses IDOR. Tuzatish: token'ni `active`dan qat'i nazar olish yoki Telegram kanali bor-u tokensiz bo'lsa fail-closed. **Eslatma:** Telegramsiz tenantlar storefront'ini buzmaslik uchun ehtiyot bo'lish kerak.
