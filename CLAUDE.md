...
- ✅ **Public API v1** — tashqi mijoz websaytlari uchun barqaror, API-kalit himoyalangan kontrakt (`/api/v1`). 6 endpoint: `GET /categories`, `/products` (filtr/sort/sahifalash), `/products/{slug}`, `/products/{id}/upsells` (ProductAddon), `/promotions` (free shipping), `POST /orders` (server-side narx, atomik stock, WEBSITE kanali, SSE/webhook). Bearer `sf_...` → tenant (`authenticateApiKey`). `?locale=uz|ru|en`. Pul butun UZS, rasm absolyut HTTPS, GET'lar 300s kesh. Shakl: `lib/public-shape.ts`, kontrakt: **`PUBLIC_API.md`**. Kalit: Sozlamalar → API yoki `npm run create-api-key -- <slug>`.
  - Schema o'zgarishi: `Product.slug` (tenant ichida unique, URL identifikatori) + `Product.origin` (filtr) + `Product.content` (JSON — tagline/highlights/benefits/ingredients/howToUse/faq/servings/bespoke). Slug create'da avto-generatsiya + `npm run backfill-slugs` (mavjudlar). **Deploy'da versiyalangan Prisma migration runner ishlaydi.** Boy kontent admin UI — ✅ bajarildi (`ProductContentEditor`, mahsulot formasida).
- ✅ **B2B narx pog'onalari (hajm bo'yicha) + MOQ** — ulgurji savdo: hajm oshgani sari narx arzonlashadi. `Product.priceTiers` (JSON `[{minQty, price}]`) + `moq` + `unit`. **Maʼlumot xarakterida** — mijozga ko'rsatiladi, B2B'da yakuniy narxni menejer tasdiqlaydi (`lib/price-tier.ts` — `priceForQty`/`parsePriceTiers`, yagona qoida manbai). Admin: mahsulot formasida "Ulgurji narx" bo'limi. Storefront PDP: pog'ona jadvali + MOQ chizig'i. Bot mahsulot kartochkasi: "Hajm bo'yicha narx" ro'yxati. Public API v1'ga `priceTiers`/`moq`/`unit` qo'shildi. **Ixtiyoriy** — bo'sh bo'lsa oddiy narx ishlaydi, migratsiya faqat ustun qo'shadi. **Deploy'da migratsiya kerak** (`20260805150000_add_b2b_price_tiers`).
